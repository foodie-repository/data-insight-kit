...
## 흩어진 회사 문서에서 팩 만들기 (스캔 → 초안 → 인터뷰 → 반영)

위 "만드는 순서"는 빈 템플릿을 손으로 채우는 흐름입니다. 도메인 지식이 기준서·정의
문서(HWP·PPT·Word)나 표(Excel·CSV)로 이미 흩어져 있다면, 그 원본에서 팩을
**증류**하는 아래 절차를 따릅니다. 원본을 팩에 통째로 붓지 말고, "정의·함정·규칙"만
남깁니다.

**3층으로 나눠 둡니다** (원본을 팩 파일에 붓지 않기 위함):

```text
domains/my-domain/
  *.md            지식층: 용어·지표·함정·규칙만 (terminology.md 등 기존 7파일)
  references/     문서층: 사람이 읽을 원본 또는 md 변환본. 팩에서 포인터로 연결
  data/           데이터층: 행 많은 표는 Parquet으로. 팩에는 스키마 의미만 기록
```

- **사람이 읽을 문서**(기준서·PPT·Word): [markitdown](https://github.com/microsoft/markitdown)으로
  md 변환 후 `references/`에. 로컬 실행이라 기밀 문서에도 안전. HWP는 `hwp` 스킬(kordoc) 사용.
- **집계·필터할 표**(행 많은 Excel·CSV): md로 바꾸지 말고 Polars로 Parquet 변환
  (`pl.read_excel(...).write_parquet(...)` — Excel 엔진 **fastexcel** 필요) 후 `data/`에.
  에이전트는 표를 읽지 않고 **쿼리**하므로, 팩에는 컬럼 의미·함정만 적으면 됩니다.
- 판단 기준: **사람이 읽으면 md, 집계하면 Parquet.** 둘 다면 둘 다.

**저작 워크플로 (초안 작성 시 이 계약을 따릅니다)**:

1. **스캔**: `references/`와 `data/`를 읽고 용어·지표·컬럼 후보를 뽑습니다.
   문서 간 정의가 어긋나면 따로 표시합니다.
2. **초안**: 템플릿 7파일에 채우되 **원본에서 확인된 것만** 값으로 씁니다.
   확신 없는 항목은 추측으로 채우지 말고 `[확인 필요]`로 표시합니다.
3. **인터뷰**: `[확인 필요]`·불일치 항목을 **질문으로 사용자(도메인 전문가)에게
   제시하고 답을 기다립니다**. 사용자 확정 없이 정의를 확정하지 않습니다 — 팩은
   이후 모든 분석의 진실 원천이라, 틀린 정의는 모든 분석을 조용히 틀리게 합니다.
4. **반영**: 답변을 반영해 완성하고, 각 항목에 출처·버전을 남깁니다
   (예: `출처: 품질기준서_v3.2, 2026-06`). 미해결 항목은 지우지 말고 `TBD`로 둡니다.

**의존성 계약**: 위 도구(markitdown, polars+fastexcel, duckdb)는 실행 PC에 없을 수
있습니다. 사용 직전 가용성을 확인하고(예: `python3 -c "import polars, fastexcel"`),
미설치면 **사용자에게 알리고 동의를 받은 뒤** 설치합니다. 동의 없는 자동 설치는
하지 않으며, 설치 실패(사내망 차단 등) 시 수동 방법을 안내하고 해당 경로를 중단합니다.

**⚠ 보안**: 회사 기밀이 담긴 팩·`references/`·`data/`는 GitHub 등 외부 저장소에
올리지 않습니다(개인 private repo 포함). 로컬 또는 사내망 저장소만 사용합니다.
빈 `domains/template/`은 공유해도 무방합니다.
